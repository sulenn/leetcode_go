...
func getMaxLen(arr [][]int, latest []int, max int) int {   // 获得当前字符串和前面字符串的差值
	for i:=0; i<len(arr); i++ {
		if latest[0] - arr[i][0] <= max {return 0}
		if (latest[1] - arr[i][1]) % 2 != 0 {continue}
		if (latest[2] - arr[i][2]) % 2 != 0 {continue}
		if (latest[3] - arr[i][3]) % 2 != 0 {continue}
		if (latest[4] - arr[i][4]) % 2 != 0 {continue}
		if (latest[5] - arr[i][5]) % 2 != 0 {continue}
